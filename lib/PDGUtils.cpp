...
std::set<Value*> pdg::pdgutils::computeAddrTakenVarsFromArg(Argument* arg)
{
    std::set<Value*> addr_taken_vars;
    for (auto user : arg->users()) {
        // if (isa<LoadInst>(user))
        if (isa<DbgValueInst>(user) || isa<DbgDeclareInst>(user))
            continue;
...
std::set<Value*> pdg::pdgutils::computeAliasForRetVal(Value& val, Function& F)
{
    ...
    while (!val_queue.empty()) {
        ...
        for (auto instI = inst_begin(F); instI != inst_end(F); ++instI) {
            if (cur_val == &*instI)
                continue;
            if (queryAliasUnderApproximate(*cur_val, *instI) != llvm::AliasResult::NoAlias) {
...
bool pdg::pdgutils::isArgTreeNode(Node *node) {
    if (node->getNodeType() == GraphNodeType::FORMAL_IN ||
        node->getNodeType() == GraphNodeType::FORMAL_OUT ||
        node->getNodeType() == GraphNodeType::ACTUAL_IN ||
        node->getNodeType() == GraphNodeType::ACTUAL_OUT)
        return true;
    return false;
}

void pdg::pdgutils::dumpArgType(Node *node) {
